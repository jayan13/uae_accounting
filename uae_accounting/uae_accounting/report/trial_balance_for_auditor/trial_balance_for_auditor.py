...
def get_data(filters):
	accounts = frappe.db.sql(
		"""select name, account_number, parent_account,is_group, account_name, root_type, report_type, lft, rgt
...
def prepare_data(accounts, filters, parent_children_map, company_currency):
	...
	for d in accounts:
		...
		if (not row.get('parent_account') and row.get('is_group')) or (not row.get('is_group') and row.get('parent_account')):
			data.append(row)
