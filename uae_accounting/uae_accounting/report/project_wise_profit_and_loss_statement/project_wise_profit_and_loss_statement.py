...
def execute(filters=None):
	...
	for pjt in projects:
		ky=pjt.strip().replace(" ", "_").replace("-", "_")
		project_list.append(frappe._dict({"key": ky, "label": pjt,"project":pjt,"from_date":year_start_date,"to_date":year_end_date,"year_start_date": year_start_date,"year_end_date": year_end_date}))
	...
	net_profit_loss = get_net_profit_loss(
		income, expense, period_list, project_list, filters.company, filters.presentation_currency
	)
	...
	columns = get_columns(
		filters.periodicity, period_list, project_list, filters.accumulated_values, filters.company
	)
	...
	report_summary = get_report_summary(
		period_list, project_list, filters.periodicity, income, expense, net_profit_loss, currency, filters
	)
...
def get_report_summary(
	period_list, project_list, periodicity, income, expense, net_profit_loss, currency, filters, consolidated=False
):
	...
	if len(project_list): #custom 
		for period in project_list:
...
def get_net_profit_loss(income, expense, period_list, project_list, company, currency=None, consolidated=False):
	total = 0
	net_profit_loss = {
...
def get_projects(filters):
	...
	if filters.get('project'):
		...
		pjts=frappe.db.sql(
		"""select DISTINCT project from `tabGL Entry` where
		posting_date between %(start_date)s and %(end_date)s and company=%(company)s and project is not null order by project""",
		{"start_date": year_start_date, "end_date": year_end_date,'company': filters.company},
		as_dict=1)
		for p in pjts:
			projects.append(p.project)
		#projects=frappe.db.get_list('Project',filters={'company': filters.company},order_by='name' ,pluck='name')
...
def get_data(
	...
	accumulate_values_into_parents(accounts, accounts_by_name, period_list,project_list)
	out = prepare_data(accounts, balance_must_be, period_list,project_list, company_currency)
	out = filter_out_zero_value_rows(out, parent_children_map)

	if out and total:
		add_total_row(out, root_type, balance_must_be, period_list,project_list, company_currency)
		...
		for entry in entries:
			...
			if len(project_list): #custom 
				for period in project_list:
					...
					if entry.project == period.project:
						
							d[period.key] = d.get(period.key, 0.0) + flt(entry.debit) - flt(entry.credit)

				if entry.posting_date < project_list[0].year_start_date:
...
def accumulate_values_into_parents(accounts, accounts_by_name, period_list,project_list):
...
def prepare_data(accounts, balance_must_be, period_list,project_list, company_currency):
	data = []
...
def add_total_row(out, root_type, balance_must_be, period_list,project_list, company_currency):
	total_row = {
		...
		if not row.get("parent_account"):
			if len(project_list):
				for project in project_list:
					total_row.setdefault(project.key, 0.0)
					total_row[project.key] += row.get(project.key, 0.0)
			else:
				for period in period_list:
...
def get_columns(periodicity, period_list, project_list, accumulated_values=1, company=None):
	columns = [
		{
			"fieldname": "account",
			"label": _("Account"),
	...
	if periodicity != "Yearly" or len(project_list):
		if not accumulated_values:
			columns.append(
				{
					"fieldname": "total",
